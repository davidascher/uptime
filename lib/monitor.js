 ...
 * @api   public
 */
function Monitor(config, serverconfig) {
...
Monitor.prototype.findChecksNeedingPoll = function(callback) {
  var api = url.parse(this.apiUrl + '/checks/needingPoll');
  var self = this;
  http.get(api, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(this.apiUrl + '/checks/needingPoll resource responded with error code: ' + res.statusCode));
    }
    var body = '';
    ...
    callback(new Error(this.apiUrl + '/checks/needingPoll resource not available: ' + e.message));
  });
};
...
Monitor.prototype.createPing = function(check, error, timestamp, time, callback) {
  ...
  var req = http.request(options, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(this.apiUrl + '/pings resource responded with error code: ' + res.statusCode));
    }
    var body = '';
    res.on('data', function(chunk) {
    body += chunk;
    ...
    callback(new Error(this.apiUrl + '/pings resource not available: ' + e.message));
  });
  req.write(postData);
  req.end();
}
 ...
 * @api   public
 */
exports.createMonitor = function(config, serverconfig) {
  return new Monitor(config, serverconfig);
}
