...
// instead of this.config.apiUrl

Monitor.prototype.findChecksNeedingPoll = function(callback) {
  var api = url.parse(apiUrl + '/checks/needingPoll');
  var self = this;
  http.get(api, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(apiUrl + '/checks/needingPoll resource responded with error code: ' + res.statusCode));
    }
    var body = '';
    ...
    callback(new Error(apiUrl + '/checks/needingPoll resource not available: ' + e.message));
  });
};
...
Monitor.prototype.createPing = function(check, error, timestamp, time, callback) {
  ...
  var options = url.parse(apiUrl + '/pings');
  options.method = 'POST';
  options.headers = {
  ...
  var req = http.request(options, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(apiUrl + '/pings resource responded with error code: ' + res.statusCode));
    }
    var body = '';
    res.on('data', function(chunk) {
    body += chunk;
    ...
    callback(new Error(apiUrl + '/pings resource not available: ' + e.message));
  });
  req.write(postData);
  req.end();
}
